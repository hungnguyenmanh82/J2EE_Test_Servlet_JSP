 ...
 */
@WebServlet(description = "Hello world test", urlPatterns = { "/HelloWorld" })
public class HelloWorld extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
    	...
    	super.init(config);
    	System.out.println("---- init servlet: /HelloWorld  ");
...
//		Handler consoleHandler = new ConsoleHandler();
		...
		try {
			consoleHandler = new FileHandler("D:/MyLogFile4.log", true);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("*****HelloWorld: doGet(): /HelloWorld ");
...
//		response.sendRedirect(request.getContextPath()+ "/SessionTrack");
		...
		URL location = HelloWorld.class.getProtectionDomain().getCodeSource().getLocation();
		...
		logger.severe("abc");
		logger.warning("abc");
		logger.config(new Exception("error here").toString());
